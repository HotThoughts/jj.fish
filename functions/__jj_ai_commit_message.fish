    ...
    switch $tool
        ...
        case cursor cursor-agent
            if test (count $argv) -gt 0
                echo $argv[1] | cursor-agent "$prompt" 2>/dev/null | tail -n 1
            else
                cat | cursor-agent "$prompt" 2>/dev/null | tail -n 1
            end
        case claude
            if test (count $argv) -gt 0
                echo $argv[1] | claude "$prompt" 2>/dev/null | tail -n 1
            else
                cat | claude "$prompt" 2>/dev/null | tail -n 1
            end
        case '*'
